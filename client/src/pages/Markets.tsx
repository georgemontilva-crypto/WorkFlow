...
import { TrendingUp, TrendingDown, ArrowRightLeft, Target, ChevronDown, Plus, Trash2, Wallet, X, Wallet2, Bell, Search, BarChart3 } from 'lucide-react';
      ...
      <div className="max-w-[1600px] mx-auto p-4 md:p-6 lg:p-8 space-y-6">
        ...
        <div className="bg-[#0A0A0A] border border-[rgba(255,255,255,0.06)] rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            {/* Columna Izquierda: Lista de Criptomonedas (40%) */}
                  ...
                  {cryptos
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => {
                            if (!activeCharts.includes(crypto.id)) {
                              setActiveCharts([...activeCharts, crypto.id]);
                            }
                          }}
                          className={`w-8 h-8 flex items-center justify-center rounded-lg transition-all flex-shrink-0 ${
                            activeCharts.includes(crypto.id)
                              ? 'bg-[#C4FF3D]/20 text-[#C4FF3D]'
                              : 'hover:bg-[rgba(255,255,255,0.05)] text-[#8B92A8] group-hover:text-[#C4FF3D]'
                          }`}
                          title="Ver gráfica"
                        >
                        ...
                        <button
                          ...
                          className="w-8 h-8 flex items-center justify-center hover:bg-[rgba(255,255,255,0.05)] rounded-lg transition-all flex-shrink-0"
            ...
            {/* Columna Derecha: Widgets de Gráficas (60%) */}
            <div className="lg:col-span-3">
              {activeCharts.length === 0 ? (
                <div className="h-full min-h-[400px] flex items-center justify-center border-2 border-dashed border-[rgba(255,255,255,0.06)] rounded-xl">
                  <div className="text-center text-[#8B92A8] p-8">
                    <BarChart3 className="w-12 h-12 mx-auto mb-3 opacity-50" />
                    <p className="text-sm">Haz clic en el icono de gráfica</p>
                    <p className="text-sm">para agregar widgets aquí</p>
                  </div>
                </div>
              ) : (
                <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-[rgba(255,255,255,0.1)] scrollbar-track-transparent hover:scrollbar-thumb-[rgba(255,255,255,0.2)]">
                  {activeCharts.map((cryptoId) => {
                    ...
                    return (
                      <div key={cryptoId} className="bg-[#121212] border border-[rgba(255,255,255,0.06)] rounded-xl p-4">
                        {/* Header del widget */}
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <button
                              onClick={() => setActiveCharts(activeCharts.filter(id => id !== cryptoId))}
                              className="w-8 h-8 flex items-center justify-center hover:bg-[rgba(255,255,255,0.05)] rounded-lg transition-all"
                              title="Cerrar gráfica"
                            >
                              <X className="w-4 h-4 text-[#8B92A8] hover:text-red-500" />
                            </button>
                          </div>
                        </div>
